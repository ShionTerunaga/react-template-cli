...
const relativeTargets = [["next", "app"], ["next", "pages"], ["tanstack-router"]] as const;
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      await fs.symlink(target, destPath).catch(() => {
        throw new Error(`failed to copy symbolic link: ${srcPath} -> ${destPath}`);
      });
    }
  }
}
...
async function copyPart(kind: UiKind, targetParts: readonly string[]) {
  ...
  const destBase = path.join(root, "packages", "main-template", "react", ...targetParts, kind);
